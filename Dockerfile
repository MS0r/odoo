...
# Create odoo user
RUN useradd --system --home /var/lib/odoo --shell /bin/bash odoo

# Install Odoo source directly
WORKDIR /opt/odoo
COPY --chown=odoo:odoo . .

# Install Python dependencies from Odoo's own requirements
...
CMD ["python3", "/opt/odoo/odoo-bin", "--config=${ODOO_RC}"]